...
namespace WebUI4CSharp_net48
{
    ...
    public class WebUIEvent
    {
        ...
        /// </summary>
        public bool Initialized { get => _event.window.ToUInt64() > 0; }
        ...
        public MemoryStream GetStream()
        {
            if (Initialized)
            {
                ...
                // Since we're on x64, convert directly to a long (which is 64-bit)
                long bufferSizeLong = (long)bufferSize.ToUInt64();

                // Ensure the size can fit into an int for MemoryStream
                if (bufferSizeLong > int.MaxValue)
                {
                    throw new InvalidOperationException("Buffer size is too large.");
                }

                MemoryStream stream = new MemoryStream((int)bufferSizeLong);
                for (long i = 0; i < bufferSizeLong; i++)
                {
        ...
        public MemoryStream GetStreamAt(UIntPtr index)
        {
            if (Initialized)
            {
                ...
                // Convert UIntPtr to a long since we're on x64
                long bufferSizeLong = (long)bufferSize.ToUInt64();
