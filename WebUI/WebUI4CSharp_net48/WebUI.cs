...
namespace WebUI4CSharp_net48
{
    ...
    public static class WebUI
    {
        ...
        public static void AddWindow(WebUIWindow newWindow)
        {
            lock(_lockObj)
            {
                ...
                for (int i = _windowList.Count - 1; i >= 0; i--)
                {
                    ...
                    if (! windowRef.TryGetTarget(out WebUIWindow window))
                    {
        ...
        public static WebUIWindow SearchWindow(UIntPtr windowId)
        {
            if (windowId.ToUInt64() > 0)
            {
                lock (_lockObj)
                {
                    ...
                    {
                        windowRef = _windowList[i];
                        if (windowRef.TryGetTarget(out WebUIWindow window) && (window.Id == windowId))
                        {
                            return window;
                        }
                    }
        ...
        {
            if (srcString == IntPtr.Zero)
            {
                return null;
            }

            int length = 0;
            while (Marshal.ReadByte(srcString, length) != 0)
            {
                length++;
            }

            byte[] buffer = new byte[length];
            Marshal.Copy(srcString, buffer, 0, length);
